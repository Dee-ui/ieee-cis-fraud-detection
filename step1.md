...
| ID | Decision | Why |
|----|----------|-----|
| D-01 | Dataset is IEEE-CIS Fraud Detection from Kaggle | 590,540 training rows with about 3.5% fraud, roughly 20,600 fraud cases. NovaPay had 193 fraud cases in 9,940 rows, which was not enough signal to train a strong model. |
| D-02 | Project name and repo name: `ieee-cis-fraud-detection` | Descriptive and instantly readable to a recruiter or hiring manager scanning your GitHub. If you prefer a product-style name like NovaPay, change it now, before the first push. It only appears in the folder name and the README. |
...
| D-07 | Paths resolved dynamically in `config/config.py` | NovaPay hardcoded an absolute Windows path. That breaks on any other machine, inside Docker, and in CI. We compute the project root from the file's own location instead. |
...
You need about **4 GB free**: roughly 1.4 GB for the data, roughly 2.5 GB for the Python environment (the machine learning libraries are large).
...
The command below creates a hidden folder called `.kaggle` in your Windows user directory. `$env:USERPROFILE` is a built-in variable that expands to `C:\Users\YourName`, so this works no matter what your username is.
...
│   ├── interim/                    # joined and cleaned, mid-pipeline   (Step 2/3)
...
└── run.py                          # single pipeline entry point       (Step 3)
```
...
### 4.3 Create the scaffold

Pick where the project lives. Avoid folders that sync to OneDrive if you can, because file locking during large writes can cause strange errors. A simple path like `C:\projects` is ideal.

The first block creates the parent folder and moves into it. `-Force` means "do not complain if it already exists".
...
These settings make VS Code use the project's own Python environment automatically and behave consistently. Comments explain each line.
...
```powershell
# Point the local repo at GitHub (replace YOUR_USERNAME)
git remote add origin https://github.com/YOUR_USERNAME/ieee-cis-fraud-detection.git
...
**Expected output:** your prompt gains a `(.venv)` prefix, like `(.venv) PS C:\projects\ieee-cis-fraud-detection>`.
...
The download is roughly 120 MB compressed and expands to roughly 1.2 to 1.4 GB.

Two things to know now, because they cause confusion later:

1. **Not every transaction has an identity record.** Only about 144,000 of 590,000 do. The join in Step 2 is a left join, and the missing identity data is itself informative rather than a defect.
...
```
1. FILE PRESENCE
  OK       train_transaction.csv            652.1 MB
  ...
...
Save this output. Paste it back to me when you start Step 2, since the exact numbers shape the EDA.
...
Tick every box before starting Step 2.

**Kaggle**
- [ ] Kaggle account exists and is logged in
- [ ] Competition rules accepted at the IEEE-CIS rules page
- [ ] The competition data page shows files rather than a rules prompt
- [ ] Credentials configured (`kaggle auth login` or `kaggle.json` in place)
- [ ] `kaggle.json` is outside the project folder

**Scaffold**
- [ ] Project folder created at a path without OneDrive syncing, ideally
- [ ] `tree /A` output matches the structure in Section 4.1
- [ ] `__init__.py` files exist in `config`, `src`, and each `src` subfolder
- [ ] `.gitkeep` files exist in every otherwise-empty folder

**Git and GitHub**
- [ ] `.gitignore` exists with a leading dot and correct contents
- [ ] `README.md` and `LICENSE` exist
- [ ] `git log` shows commits
- [ ] Public GitHub repository exists and the README renders on it
- [ ] `git status` shows no CSV files
- [ ] Branch `step-01-foundations` created, pushed, merged into `main`
- [ ] Tag `v0.1.0-step1` pushed

**Environment**
- [ ] `.venv` folder exists
- [ ] Terminal prompt shows `(.venv)` when activated
- [ ] `python --version` prints 3.11.x
- [ ] `python -c "import pandas, sklearn, lightgbm, xgboost, catboost, mlflow, shap; print('all imports OK')"` succeeds
- [ ] `requirements.lock.txt` exists
- [ ] VS Code interpreter set to `.venv`

**Data**
- [ ] Five CSV files present in `data/raw/`
- [ ] `python scripts/verify_data.py` prints VERIFICATION PASSED
- [ ] `reports/data_inventory.md` exists
- [ ] Fraud rate reported at roughly 3.5%
...
| `ModuleNotFoundError: No module named 'config'` | Running from the wrong folder | `Set-Location` back to the project root, which is the folder containing `README.md` |
...
| Odd file locking or permission errors | Project sits inside a OneDrive-synced folder | Move the project to `C:\projects\` |
...
## 13. What to send me before Step 2

Paste these three things into your next message:

1. The full output of `python scripts/verify_data.py`
2. The output of this version check, so I write Step 2 code against the exact libraries you have:

```powershell
pip list | Select-String -Pattern "pandas|numpy|scikit-learn|lightgbm|xgboost|catboost|mlflow|shap|pyarrow"
```

3. Your GitHub repository URL

If anything in the checklist did not tick, say which item and what the error was. Do not work around it silently, because Step 2 assumes this foundation is solid.

---

## 14. What Step 2 will cover

- Joining `train_transaction` and `train_identity`, and what to do about the 76% of transactions with no identity record
- Grouping the 394 columns into meaningful families: `C` counting features, `D` timedelta features, `M` match flags, `V` engineered Vesta features, plus the identity block
...
- Missing value patterns, including the columns that are more than 90% empty
- Memory reduction, taking the training table from roughly 1.9 GB down to roughly 550 MB by choosing smaller numeric types
- Saving the joined table as Parquet in `data/interim/`
- Producing the first charts into `reports/figures/`

---

*End of Step 1. `PROJECT_STATE.md` follows as a separate document.*
